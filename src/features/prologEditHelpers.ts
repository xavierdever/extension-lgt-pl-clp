...
export function prologLoadEditHelpers(subscriptions: Disposable[]) {
  subscriptions.push(
  ...
  function prologGetPreviousClauseHead(doc: TextDocument, line: number): string {
    if (line <= 0) {
      return "";
    }
    ...
    return prologGetPreviousClauseHead(doc, line - 1);
  }

  function isRecursiveProlog(doc: TextDocument, line: number) {
    if (line <= 0) {
      return false;
    }
    let i = line - 1;
  ...
  function nextRecursiceParamsProlog(
    doc: TextDocument,
    line: number,
    originalHead: string
  ): string {
  ...
  workspace.onDidChangeTextDocument(
    e => {
      ...
      if (lastChar === "_") {
        ...
        let prevHead: string = prologGetPreviousClauseHead(e.document, line - 1);
        if (isRecursiveProlog(e.document, line)) {
          prevHead = nextRecursiceParamsProlog(e.document, line - 1, prevHead);
        }
        editor
          .edit(edit => {
            edit.replace(
