...
export class PrologDefinitionProvider implements DefinitionProvider {
  public provideDefinition(
    ...
    let pred = Utils.prologGetPredicateUnderCursor(doc, position);
    if (!pred) {
      return null;
    }

    let exec = Utils.PROLOG_RUNTIMEPATH;
    let args: string[] = [],
      prologCode: string,
      result: string[],
      predToFind: string,
    ...
    switch (Utils.DIALECT) {
      ...
      case "ecl":
        ...
        if (doc.isDirty) {
          doc.save().then(_ => {
            let syncPro = cp.spawnSync(Utils.PROLOG_RUNTIMEPATH, args, runOptions);
            if (syncPro.status === 0) {
